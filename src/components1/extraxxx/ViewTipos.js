...
import {
  getCurrentTipos,deleteCurrentTipos,updateCurrentTipos,buscarCurrentTipos
...
class ViewTipos extends Component {
  constructor(){
    super();
    this.state={
      tipoData:[],
  ...
  toggleModal = (id) => {
   ...
   this.props.updateCurrentTipos(this.props.buscafile._id,tipoData);
   this.toggleModalfin();
...
//    console.log(lolo)
 ...
 //   console.log(this.state.term)
  ...
  render() {
    ...
    const pageNumbers = [];
    for (let i = 1; i <= Math.ceil(tiposfile.length / todosPerPage); i++) {
      pageNumbers.push(i);
    }
  ...
  console.log("filtrando resultados ")
    let filteredContacts = tiposfile.filter(
      (task) =>{
  ...
  return  poetName.indexOf(this.state.search) !== -1;    
   ...
   // console.log(filteredContacts);
    ...
    if (filteredContacts === null || loading) {
      ...
      if (Object.keys(this.props.tiposfile).length > 0) {
    ...
    return (
      ...
      <div clasname="container colo">   
        <h4 className="mb-4 titulo">Tipos de Documento</h4>
        <div className="row">
...
ViewTipos.propTypes = {
  ...
  updateCurrentTipos: PropTypes.func.isRequired,
  buscarCurrentTipos: PropTypes.func.isRequired,
...
const mapStateToProps = state => ({
  ...
  buscafile: state.tiposfile.buscafile,
  errors: state.errors
  
}
);

export default connect(mapStateToProps, { getCurrentTipos ,deleteCurrentTipos,updateCurrentTipos,buscarCurrentTipos })(
  ViewTipos
);
