...
import {
  getCurrentLineasql,deleteCurrentLineasql,updateCurrentLineasql  
...
class ViewLinea extends Component {
  constructor(){
    super();
    this.state={
      tipoData:[],
  ...
  toggleModal = (id,cod,des) => {
   ...
   onSubmit(e) {
    ...
    this.props.updateCurrentLineasql(this.state.codigo,tipoData);
    this.toggleModalfin();
  ...
  onDeleteClick(id) {
    this.props.deleteCurrentLineasql(id);
    }

 

  componentDidMount() {
     this.props.getCurrentLineasql();
    }
  ...
  render() {
    ...
    const classes = useStyles;
            ...
            const { lineafiles, loading } = this.props.lineafile;
            const { currentPage, todosPerPage } = this.state;
            const indexOfLastTodo = currentPage * todosPerPage;
            const indexOfFirstTodo = indexOfLastTodo - todosPerPage;            
            const pageNumbers = [];
            for (let i = 1; i <= Math.ceil(lineafiles.length / todosPerPage); i++) {
              pageNumbers.push(i);
            }
            let filteredContacts = lineafiles.filter(
              (task) =>{
            ...
            if (Object.keys(this.props.lineafile).length > 0) {
              dashboardContent = (          
              currentTodos.map(l1 => (
...
ViewLinea.propTypes = {
  getCurrentLineasql : PropTypes.func.isRequired,
  deleteCurrentLineasql: PropTypes.func.isRequired,
  updateCurrentLineasql: PropTypes.func.isRequired,

};

const mapStateToProps = state => ({
  lineafile: state.lineafile,  
  errors: state.errors
}
);

export default connect(mapStateToProps, { getCurrentLineasql ,deleteCurrentLineasql,updateCurrentLineasql })(
  ViewLinea
);
