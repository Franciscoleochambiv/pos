...
import {
  getCurrentSeriesql,deleteCurrentSeriesql,updateCurrentSeriesql  
...
class ViewSerie extends Component {
  ...
  toggleModal = (id,cod,des) => {
   ...
   onSubmit(e) {
    ...
    this.props.updateCurrentSeriesql(this.state.codigo,tipoData);
    this.toggleModalfin();
  ...
  onDeleteClick(id) {
    this.props.deleteCurrentSeriesql(id);
    }

 

  componentDidMount() {
     this.props.getCurrentSeriesql();
    }
  ...
  render() {
    ...
    const classes = useStyles;
            ...
            const { seriefiles, loading } = this.props.seriefile;
            const { currentPage, todosPerPage } = this.state;
            const indexOfLastTodo = currentPage * todosPerPage;
            const indexOfFirstTodo = indexOfLastTodo - todosPerPage;            
            const pageNumbers = [];
            for (let i = 1; i <= Math.ceil(seriefiles.length / todosPerPage); i++) {
              pageNumbers.push(i);
            }
            let filteredContacts = seriefiles.filter(
              (task) =>{
            ...
            if (Object.keys(this.props.seriefile).length > 0) {
              dashboardContent = (          
              currentTodos.map(l1 => (
...
ViewSerie.propTypes = {
  getCurrentSeriesql : PropTypes.func.isRequired,
  deleteCurrentSeriesql: PropTypes.func.isRequired,
  updateCurrentSeriesql: PropTypes.func.isRequired,

};

const mapStateToProps = state => ({
  seriefile: state.seriefile,  
  errors: state.errors
}
);

export default connect(mapStateToProps, { getCurrentSeriesql ,deleteCurrentSeriesql,updateCurrentSeriesql })(
  ViewSerie
);
